...
describe('Create User Tests', () => {
    ...
    it('test 1',() => {
        let screen_data = {username:'user1',email:'user@test.com',dni:'12345678A', result:'', message_color:'black'}
        ...
        expect(screen_data.message_color).toBe("black")
    ...
    it('test 2',() => {
        let screen_data = {username:'user1',email:'usertest.com',dni:'12345678A', result:'', message_color:'black'}
        let user_repo = new MemoryUserRepo([])
        let cmd = new CreateUser(user_repo, screen_data, uuid_gen)
        cmd.execute()
        expect(screen_data.result).toBe("Email incorrecte")
        expect(screen_data.message_color).toBe("red")
