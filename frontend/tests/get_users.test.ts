...
describe ('Get User Tests', ()=>{
    it("test 1: GetUsers with empty user repo returns empty array", async ()=>{
        ...
        await exec
        let users = await exec;
        expect(users.length).toBe(0);
        expect(screen_data.users_list).toBe('');
    });

    it("test 2: GetUsers returns the right user", async ()=>{
    ...
    it("test 3: GetUsers returns the right user", async ()=>{
        let u = new User("1", "u1", "u1@test.com", "12345678A");
